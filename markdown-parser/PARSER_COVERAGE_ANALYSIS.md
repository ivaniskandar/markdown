...
| 优先级 | 项目 | 说明 |
|--------|------|------|
| **P3** | 自动生成标题 ID | 基于标题文本自动生成锚点 ID |
| **P3** | 标题锚点链接 | `[link](#id)` 跳转支持 |
| **P4** | 语法高亮 | 渲染器职责，根据 info string 进行代码着色 |
| **P4** | 表格列数规范化 | 列数以分隔行为准，不足补空，多余截断 |
| **P4** | 表格不打断段落 | 需空行隔开表格和前面的段落 |
| **P4** | GFM 禁止 HTML | 过滤危险 HTML 标签 |
| **P4** | 链接引用标题跨行 | 标题可换行书写 |
| **P5** | URL 百分号编码 | 链接中特殊字符自动编码 |
| **P5** | TOC 自动生成 | `[TOC]` / `[[toc]]` 目录生成 |
| **P5** | 可折叠内容 | `<details>` HTML 块支持 |
| **P6** | 缩写定义 | `*[abbr]: Full Text` |
| **P6** | 键盘按键 | `<kbd>` 标签支持 |
